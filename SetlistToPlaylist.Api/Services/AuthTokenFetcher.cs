...
public class AuthTokenFetcher : IAuthTokenFetcher
{
    ...
    public async Task<SpotifyAuth> GetSpotifyAuthFromSession()
    {
        ...
        var auth = JsonConvert.DeserializeObject<SpotifyAuth>(authString) ?? throw new SerializationException($"Unable to deserialize spotify auth token string {authString}");
